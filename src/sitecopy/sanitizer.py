...
def _looks_like_prose(tag_text: str | None) -> bool:
    ...
    Real markup either has no spaces (`<div>`, `<br/>`) or carries `name="value"`
    attributes. Words separated by spaces with no `=` anywhere is someone writing
    between angle brackets.
    """
    if not tag_text:
        return False
    inner = tag_text[1:-1].rstrip("/")
    return "=" not in inner and len(inner.split()) > 1
...
class _Sanitizer(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag not in ALLOWED_TAGS:
            if _looks_like_prose(self.get_starttag_text()):
                # "<de lunes a viernes de 9 a 18 hs>" parses as a tag, so dropping it
                # deleted the whole phrase — silently, since it has no text content for
                # the loss guard to miss. Escaped, so it can never come back as markup.
                self._out.append(escape(self.get_starttag_text() or "", quote=False))
            return  # otherwise drop the tag, keep the text inside it
